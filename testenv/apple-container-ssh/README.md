...
Each worker gets 1 CPU / 3.5GB (`DSKQ_APPLE_WORKER_CPUS` /
`DSKQ_APPLE_WORKER_MEMORY` override), so the pair together matches one
`ubuntu-latest` CI runner (2 CPU / 7GB).

SSH aliases after `up.sh`:

- `dskq-w1` → worker IP, port 22, user `dev`
- `dskq-w2` → the other worker
...
```bash
container image rm local/dskq-linux-ssh-worker:latest
```
