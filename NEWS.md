...
- `--on HOST` before the verb (`--on HOST status` / `submit` / `cancel`) picks the controller. Remote Julia uses Kit auto-detect; `--remote-julia` / `JULIA_DISTRIBUTED_EXE` override. The orderer stays stateless.
