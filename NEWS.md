...
- CLI `add-host` / `remove-host` write config `hosts` on the queue host (not via `qhost:`). Kit SSH names: `parent`, `host1`, or `child:host1` → `host1`. First add creates the list. Missing key stays allow-all. Empty array allows none. A leftover `allowed` key is still read until rewritten. Hand-edit still works. CLI `submit` reads the file; library `submit!` uses `Queue(; allowed=…)`. `:N` stays on the job.
