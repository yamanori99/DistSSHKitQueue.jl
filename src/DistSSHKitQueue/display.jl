...
function print_queue_usage(io::IO=stdout)
    ...
    DistSSHKit.print_help_lines(io,
        ...
        "  list-host                      Host tokens from config allowed; ssh -G Host / HostName / User / Port",
        "  add-host NAME [NAME...]        Add Kit names to config allowed (first add creates the list)",
    ...
    DistSSHKit.print_help_lines(io,
        ...
        "  add-host / remove-host write that allowed list (same qhost: forwarding). Names: parent or host1 / child:host1.",
        ...
        "  config allowed = [\"parent\", \"host1\"] (or child:host1). Library submit! uses Queue(; allowed=…).",
