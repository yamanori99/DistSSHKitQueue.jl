...
SSH Host `dskq-w1` / `dskq-w2` vs Compose/DNS `child-1` / `child-2` is leftover (`dskq-` so Kit's stack can coexist). Aligning those names (`child-1` / `host1`, loopback `qh`) is naming review ([#35](https://github.com/yamanori99/DistSSHQueue.jl/issues/35)), not a 0.1.0 gate. Three physical hosts is dedicated-host smoke ([#46](https://github.com/yamanori99/DistSSHQueue.jl/issues/46)), not this file.
