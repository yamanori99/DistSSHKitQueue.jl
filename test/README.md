...
That is `test/runtests.jl` (unit + integration). It tags autoserve `serve` pids and SIGTERMs them on exit or if `Pkg.test`'s parent dies, so Ctrl-C does not leave `nohup` `serve` on the laptop. Real SSH:
...
| e2e | docker-ssh workers, `serve` API, and `-m DistSSHKitQueue` (`qhost:` over loopback OpenSSH) | local-only CLI wiring |
...
`watch` is a live status table (Ctrl-C), not a job monitor. Finite frames in tests use `DISTSSHKITQUEUE_WATCH_TICKS` (not `--ticks`). A later monitor package may take the name `watch` ([#35](https://github.com/yamanori99/DistSSHKitQueue.jl/issues/35)).
